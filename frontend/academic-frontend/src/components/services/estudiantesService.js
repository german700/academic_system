...
const API_URL = `${BASE_URL}/api/academic/students/`;
const API_COURSES_URL = `${BASE_URL}/api/academic/courses/`;
...
// Helper para fetch con manejo de errores y JSON
...
export const obtenerMateriaEstudiante = async (studentId, materiaId) => {
  const response = await fetch(`${BASE_URL}/api/academic/students/${studentId}/materia/${materiaId}/`, {
    method: "GET",
    headers: getAuthHeaders(),
  });
...
export const fetchMisNotasPorMateria = async (courseId, subjectId, period) => {
  const response = await fetch(
    `${BASE_URL}/api/academic/students/my-grades-by-subject/?course_id=${courseId}&subject_id=${subjectId}&period=${period}`,
    {
      method: "GET",
      headers: getAuthHeaders(),
    }
